SWEP.PrintName = "The Pickle Club"
SWEP.Author = "Zgrad"
SWEP.Instructions = "Bane of hotdogs everywhere."
SWEP.Category = "HOMO"
SWEP.IconOverride = "materials/items_icons/pickleicon.png"
...
SWEP.ViewModel = "models/zgrad/pickle/pickle.mdl"
SWEP.WorldModel = "models/zgrad/pickle/pickle.mdl"
...
local poleMdl = ClientsideModel("models/props_docks/dock03_pole01a_256.mdl")
-- Settings...
WorldModel:SetSkin(1)
WorldModel:SetNoDraw(true)
poleMdl:SetSkin(1)
poleMdl:SetNoDraw(true)

function SWEP:DrawWorldModel()
    local _Owner = self:GetOwner()
    local scale = 3
    if (IsValid(_Owner)) then
        local offsetVec = Vector(3.5, -1, -20)
        local offsetAng = Angle(10, 0, 90)
        local poleOffsetVec = Vector(3.5, -1, 0)
        local poleOffsetAng = Angle(-5, 0, 0)
        ...
        local newPolePos, newPoleAng = LocalToWorld(poleOffsetVec, poleOffsetAng, matrix:GetTranslation(), matrix:GetAngles())
        ...
        poleMdl:SetPos(newPolePos)
        poleMdl:SetAngles(newPoleAng)
        poleMdl:SetModelScale(0.1)
        poleMdl:SetupBones()
    else
        WorldModel:SetPos(self:GetPos())
        WorldModel:SetAngles(self:GetAngles())
        WorldModel:SetModelScale(scale)
        poleMdl:SetPos(self:GetPos())
        poleMdl:SetAngles(self:GetAngles())
        poleMdl:SetModelScale(0.1)
    end
