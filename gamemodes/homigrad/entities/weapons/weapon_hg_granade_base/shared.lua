...
local function ThrowGrenade(ply, force, grenade)
...
function SWEP:PrimaryAttack()
    if SERVER then
        ThrowGrenade( self:GetOwner(), 750, self.Grenade )
        self:Remove()
...
function SWEP:SecondaryAttack()
    if SERVER then
        ThrowGrenade( self:GetOwner(), 250, self.Grenade )
        self:Remove()
        self:GetOwner():SelectWeapon( "weapon_hands" )
    -- elseif CLIENT then
        -- Client-side logic (if any) can be added here
    end
