...
if SERVER then
    ...
    function ENT:Use(activator, activatorAgain, onOff)
        ...
        local Time = CurTime()
        if self.ShiftAltUse and Dude:KeyDown(IN_ALT1) and Dude:KeyDown(IN_SPEED) then return self:ShiftAltUse(Dude, tobool(onOff)) end

        if tobool(onOff) then
            local State = self:GetState()
            if State < 0 then return end
            local Alt = Dude:KeyDown(IN_ALT1)
