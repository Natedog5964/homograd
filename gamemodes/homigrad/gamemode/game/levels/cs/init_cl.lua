...
function css.HUDPaint_RoundLeft(white2) --позиции точек и счёт
    ...
    if startRound > 0 and lply:Alive() then
        ...
        draw.DrawText( "RUSSIAN WORDS = Point Capture", "HomigradFontBig", ScrW() / 2, ScrH() / 8, Color( 155,55,155,math.Clamp(startRound,0,1) * 255 ), TEXT_ALIGN_CENTER )
        draw.DrawText( "RUSSIAN WORDS = Capture points and score to win", "HomigradFontBig", ScrW() / 2, ScrH() / 1.2, Color( 155,155,155,math.Clamp(startRound,0,1) * 255 ), TEXT_ALIGN_CENTER )
        return
    end
